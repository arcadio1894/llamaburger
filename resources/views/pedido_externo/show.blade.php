...
@section('title','Pedido Externo #' . $atencion->id)
...
@section('content')
    <div class="card">
        ...
        <div class="alert alert-info py-2">
            Este pedido no está asociado a una mesa ni mozo. Solo admite una comanda.
        </div>
    </div>
        ...
        <div class="col-12 col-lg-4 d-none d-lg-block">
            <div id="order-panel" class="card sticky-order">
                ...
                <div class="card-footer p-2">
                    ...
                    @else
                        <form id="form-send-pay-{{ $atencion->id }}" class="w-100" method="POST"
                              action="{{ route('pedido.externo.ir_pagar', $atencion->id) }}">
                            @csrf
                            <button id="btn-send-pay-{{ $atencion->id }}" type="submit"
                                    class="btn btn-dark btn-block mt-2">
                                IR A PAGAR
                            </button>
                        </form>
                        {{--<button id="btn-send-pay" class="btn btn-dark btn-block mt-2" data-action="{{ route('pedido.externo.ir_pagar', $atencion->id) }}">IR A PAGAR</button>--}}
                    @endif
                </div>
            </div>
        </div>
    ...
    <aside class="control-sidebar control-sidebar-light" id="order-aside">
        <div class="cs-wrap">
            ...
            <div class="cs-top" id="order-aside-top">
                ...
                @if(in_array($comanda->estado, ['enviada','cocinando','servida']))
                    <button id="a-send-pay" class="btn btn-dark btn-block mt-2" disabled>IR A PAGAR</button>
                @else
                    <form id="form-send-pay-{{ $atencion->id }}" class="w-100" method="POST"
                          action="{{ route('pedido.externo.ir_pagar', $atencion->id) }}">
                        @csrf
                        <button id="a-send-pay-{{ $atencion->id }}" type="submit"
                                class="btn btn-dark btn-block mt-2">
                            IR A PAGAR
                        </button>
                    </form>
                    {{--<button id="a-send-pay" class="btn btn-dark btn-block mt-2" data-action="{{ route('pedido.externo.ir_pagar', $atencion->id) }}">IR A PAGAR</button>--}}
                @endif
            </div>
...
@section('scripts')
    ...
    <script src="{{ asset('js/pedido_externo/show.js') }}"></script>
